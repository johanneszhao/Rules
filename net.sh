...
示例:
  ...
  sudo bash proxy-net-tune.sh --hysteria-range 50000:51000
...
raise_live_proxy_limits() {
    ...
    local pid command_line process_name unit raised=0
    local -A seen_pids=()
    for unit in "${PROXY_UNITS[@]}"; do
...
validate_result() {
    local failures=0 actual key expected i unit limit
    heading "本机生效检查"
...
show_status() {
    ...
    printf '主网卡           : %s\n' "${PRIMARY_IFACE:-未检测到}"
    local key installed_kernel="" unit limit
    for key in net.ipv4.tcp_available_congestion_control net.ipv4.tcp_congestion_control \
        ...
        net.ipv4.tcp_wmem net.ipv4.tcp_fastopen net.ipv4.tcp_mem net.ipv4.tcp_tw_reuse \
