...
# 参考: https://github.com/233boy/sing-box (结构精简 + 性能优化)
...
get_ip() {
    ...
    # IPv6 加方括号
    [[ $is_addr == *:* ]] && is_ip_fmt="[$is_addr]" || is_ip_fmt="$is_addr"
}

get_port() {
    local p
    while :; do
        p=$(shuf -i 2000-6000 -n1)
        ss -tunlp 2>/dev/null | grep -q ":$p " || { echo "$p"; return; }
    done
}

gen_uuid() { "$is_core_bin" generate uuid; }
gen_pass() { "$is_core_bin" generate rand 16 --base64 2>/dev/null || openssl rand -base64 16; }
...
download_core() {
    ...
    local ver
    ver=$(_wget "https://api.github.com/repos/${is_core_repo}/releases/latest" \
        | jq -r '.tag_name')
    [[ ! $ver ]] && err "获取最新版本失败."
    is_core_ver="$ver"

    local tmp; tmp=$(mktemp -d)
    local file="$tmp/core.tar.gz"
    local link="https://github.com/${is_core_repo}/releases/download/${ver}/${is_core}-${ver:1}-linux-${is_arch}.tar.gz"
    green "下载内核: $ver"
...
EOF
    ...
    local br; bbr=$(sysctl -n net.ipv4.tcp_congestion_control)
    green "当前拥塞控制算法: $bbr"
}
...
# 通用性能内联字段 (加到每个 inbound)
inbound_perf='"tcp_fast_open": true, "tcp_multi_path": false, "sniff": true, "sniff_override_destination": false'
...
EOF
    restart_core
    green "\nAnyTLS 节点已创建:"
    echo "anytls://$pass@$is_ip_fmt:$port?insecure=1&sni=$is_anytls_sni#$name"
}
...
add_vless_ws_tls() {
    local port=443 uuid path name domain
    ...
    # sing-box >= 1.14 用 certificate_provider,否则用 acme
    local minor; minor=$(echo "$is_core_ver" | cut -d. -f2)
    local tls_block
    if [[ ${minor:-0} -ge 14 ]]; then
        tls_block="\"enabled\": true, \"server_name\": \"$domain\", \"certificate_provider\": { \"type\": \"acme\", \"domain\": [\"$domain\"] }"
    else
        tls_block="\"enabled\": true, \"server_name\": \"$domain\", \"acme\": { \"domain\": [\"$dom\"] }"
    fi
...
EOF
    restart_core
    green "\nVLES-WS-TLS 节点已创建:"
    echo "vless://$uuid@$domain:443?encryption=none&security=tls&sni=$domain&type=ws&host=$domain&path=$(echo "$path" | sed 's:/:%2F:g')#$name"
}
...
install_shortcut() {
    ...
    chmod +x "$is_dir/install.sh" "$is_sh_file"
}
...
list_nodes() {
    green "已安装节点配置:"
    ls -1 "$is_conf_dir"/*.json 2>/dev/null | grep -v 00_base | sed 's#.*/##; s#.json##' | sed 's/^/  /'
}
...
do_install() {
    ...
    green "\n安装完成! 版本: $is_core_ver"
    green "以后输入 \033[1mjsb\033[0m 打开菜单。\n"
    menu
}
